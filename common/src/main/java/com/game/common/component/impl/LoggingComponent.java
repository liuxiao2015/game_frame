...
/**
 * 日志组件示例实现。 这是一个通过 SPI 自动装配的示例组件，主要用于演示组件框架的生命周期管理。 在初始化、启动、停止阶段都会输出相应的日志信息，便于验证组件框架的正确性。
 *
 * <p>此组件的主要功能： - 在 init() 阶段输出初始化日志 - 在 start() 阶段输出启动日志 - 在 stop() 阶段输出停止日志 - 演示组件生命周期的完整流程
 *
 * <p>装配顺序设置为 100，属于较早启动的基础组件。 后续可以添加更多业务组件，通过调整 order 值来控制启动顺序。
 *
 * @author game-frame
 * @since 1.0.0
 */
